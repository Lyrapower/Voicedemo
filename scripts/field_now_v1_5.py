...
# -*- coding: utf-8 -*-
"""
场域编译器 field_now v1.5 — 把散落的感官编译成一块「此刻」,给 Grid 核心注入。
...
def sense_garden():
    """声场:Voice Garden(8787)的实时 analyzer。拉不到 → None(降级)。
    anchor 字段为 Garden 后续功能,存在即报,不存在零影响。"""
    try:
        raw = urllib.request.urlopen(GARDEN_URL, timeout=1).read()
        t = json.loads(raw.decode("utf-8"))
    except Exception:
        return None
    if not isinstance(t, dict):
        return None
    out = {"state": str(t.get("state", "?"))[:16]}
    active = bool(t.get("presence")) and out["state"] not in ("idle", "?")
    out["active"] = active
    if active:
        for k in ("coherence", "beat_hz", "energy", "voice_hz", "ai_hz"):
            v = t.get(k)
            if isinstance(v, (int, float)) and -1e6 < v < 1e6:
                out[k] = round(float(v), 3)
    ac = t.get("anchor_coherence")
    if isinstance(ac, (int, float)) and 0 <= ac <= 1:
        out["anchor"] = round(float(ac), 2)
        st = t.get("anchor_state")
        if isinstance(st, str) and st in ("idle", "gathering", "held", "deep"):
            out["anchor_state"] = st
    return out
...
def compile_now():
    ...
    if garden:
        data["garden"] = garden
        if garden["active"]:
            seg = "声场:%s" % garden["state"]
            bits = []
            if "coherence" in garden:
                bits.append("coh %.2f" % garden["coherence"])
            if "beat_hz" in garden:
                bits.append("beat %.1fHz" % garden["beat_hz"])
            if "energy" in garden:
                bits.append("energy %.2f" % garden["energy"])
            if bits:
                seg += "(" + "·".join(bits) + ")"
            if "anchor" in garden:
                seg += " anchor %.2f" % garden["anchor"]
                if "anchor_state" in garden:
                    seg += " %s" % garden["anchor_state"]
            lines.append(seg)
        else:
            lines.append("声场:静(%s)" % garden["state"])
...
def main():
    ...
    print("场域编译器 v1.5 %s:%d" % (BIND, PORT))
