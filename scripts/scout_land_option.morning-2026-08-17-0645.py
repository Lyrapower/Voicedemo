...
def main():
    ...
    review = {
        "primary_label": "glm" if glm.get("text") else "ds",
        "review_origin": origin,
        "review_ts": review_ts,
        "glm_final": glm.get("text") or "",
        "glm_meta": meta,
        "expanded_final": glm.get("text") or "",
        "expanded_meta": meta,
    }
    doc["ds"] = data
    doc["_engine"] = cross
    ...
    final.write_text(
        stamp_final_markdown(
            glm.get("text") or "(glm empty — ds json landed)",
            origin=origin,
            review_ts=review_ts,
            title=title,
        ),
    ...
    emit(day, title, body_store, jp, data, cross, glm.get("text") or "", meta)
    print(
        "[land] wrote", jp,
        "glm_chars", len(glm.get("text") or ""),
        "origin", origin,
        "ts", review_ts,
    )
