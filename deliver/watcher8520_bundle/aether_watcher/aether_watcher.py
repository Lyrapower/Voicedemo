    ...
    if kind == "price":
        ...
        for ev in st.pop("events", []):
            try:
                ...
                nexus = Path(__file__).resolve().parent.parent / "aether_nexus"
                if str(nexus) not in sys.path:
                    sys.path.insert(0, str(nexus))
                from grid_notify import publish_momentum_hit

                publish_momentum_hit(
                    sym=str(ev.get("sym") or ""),
                    note=str(ev.get("note") or ev.get("detail") or "")[:240],
                    hit=ev.get("hit") or {},
                    label=t.get("name", "MomentumSticker"),
                )
            except Exception as exc:
                logger.warning("grid momentum emit failed: %s", exc)
            events = safe_read_json(EVENTS_PATH, [])
