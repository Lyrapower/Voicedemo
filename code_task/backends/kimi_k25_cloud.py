"""Ollama Cloud Kimi K2.6 — VL bypass only (describe_image); thinking blackboxed."""
...
from code_task.kimi_input_scope import MAX_IMAGES_EXPANDED, build_task_scoped_messages  # noqa: E402
from code_task.kimi_k25_pricing import enrich_kimi_usage  # noqa: E402
...
async def execute_candidate(
    *,
    ...
    try:
        label = (task_label or "grid_candidate_task").strip()
        img_cap = MAX_IMAGES_EXPANDED if label == "grid_expanded" else None
    ...
    usage = enrich_kimi_usage(raw_usage)
    usage["latency_ms"] = int((time.time() - started) * 1000)
    usage["backend_id"] = CANDIDATE_SOURCE
...
def shadow_candidate_response(*, request_id: str, prompt: str, images: Any = None) -> CandidateResponse:
    ...
    try:
        build_task_scoped_messages(prompt=prompt, task_label="shadow", images=images)
    except ValueError as exc:
        return CandidateResponse(
            request_id=request_id,
            ok=False,
    ...
    return CandidateResponse(
        ...
        done_reason="shadow",
        usage={"mode": "shadow", "backend_id": CANDIDATE_SOURCE},
