...
def build_review(date):
    ...
    rev = {"date": date, "legs": out, "hit": "%d/%d" % (nh, len(out)),
...
def earnings_movers(payload, today):
    ...
    # 批量 Alpaca snapshot(stooq 本机已死);榜单仍只出前 8
    ordered, seen = [], set()
...
def amc_tonight(payload, today):
    ...
    # 批量 Alpaca(本机 stooq JS 墙死)
    rich = []
    tops = out[:15]
    snaps = {}
    if tops and hasattr(fetchers, "quote_layer"):
        snaps = fetchers.quote_layer.snapshot(tops) or {}
    for sym in tops:
        r = snaps.get(sym) or fetchers._stooq_daily(sym.lower() + ".us", sym, "amc_tonight") or {}
        rich.append({"symbol": sym, "rsi14": r.get("rsi14"),
                     "chg5_pct": r.get("chg5_pct"), "chg_pct": r.get("chg_pct")})
    return rich
...
def render_brief_html(date, mode, data, raw_text, cross=None):
    ...
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            ...
            '%s<footer>build scout v3.19 · 趋势引擎 · 四槽卡 · 盘后实测 · cap四层 · ET锚定 · 三层宇宙 · 复盘回路 · 无证据不点名 · '
