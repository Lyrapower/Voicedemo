#!/usr/bin/env bash
# ============================================================================
# Scout Agent · v3.28.0 · 总包(完整替代;plist/verify 保护;自检自跑)
...
# Scout Agent v3.28.0(DS 决策官 · 三班制 + BMO 伏击 + 引擎候选池 + 质量地板 + ETF 硬排 + 尾盘形态闸 + 榜硬门 + 自算趋势榜)
...
def _history(sym):
    ...
    fresh = bool(rows) and rows[-1].get("date", "") >= prev_trading_day(trading_date()).isoformat()
...
def fetch_screener_universe(cache_path=None, ttl_s=20 * 3600):
    ...
    try:
        ...
        json.dump({"ts": time.time(), "n": len(syms), "raw_n": len(rows), "symbols": syms},
...
def _s2_section(shift):
    ...
    return ("S2 财报伏击·今晚 AMC(财报班主攻位一):amc_tonight 名单择一,~12:50 PST 尾盘买入,\n"
            ...
            "   amc_tonight 已按引擎 earn_score 排,earn_why 逐项可查;call 只做 earn_score≥0 的票,\n"
            "   当日≤-1% 且 loc<0.5 的票点 call = 引擎作废;三档之上先看形态,再看档位;\n"
            "   资金确认硬规则(v3.27.1,Lyra:不在 FMP top mover 榜的不收):S2/S4 call 腿的票必须在当日\n"
            "   FMP 涨幅榜或最活跃榜(收涨)上(名单行 fmp_board=true,earn_why 带 FMP榜+2),否则引擎作废;\n"
...
- 名单已按引擎 earn_score 降序(尾盘形态分,earn_why 逐项可查;不是市值序);前三按分逐票评估,
  未点名须在 rejected 或正文给一句理由;earn_score<0 的票禁做 call(点了引擎作废);
...
- 名单已按 earn_score 降序;前三按分逐票评估,未点名须给一句理由;earn_score<0 禁做 call
...
S1 主池·引擎候选池(v3.26,2026-08-24 案:六眼只当参考、S1 由 DS 自由挑,挑出热簇里 +1.77% 的
   ...
   七眼并集(异动榜/最活跃/🔥同频团/🔥热簇/连涨/自选/趋势榜=自算全市场 5 日趋势,不靠 FMP 榜,BMNR 型)→逐票实测→硬地板(price≥${fetchers.pool_price_floor():g}、
    ...
    for k in ("amc_tonight", "bmo_tomorrow", "upcoming_earnings"):
        ...
        for x in rows:
            ...
            if cl:
                ...
                onb = x.get("symbol") in board
                x["fmp_board"] = onb
                if onb and x.get("earn_score") is not None:
                    x["earn_score"] += 2
                    x["earn_why"] = (x.get("earn_why") or "") + " FMP榜+2"
                keep.append(x)
...
def _last_closed_session(today):
    """趋势榜的键:最近一根已收盘日线的日期——收盘前 = 上一交易日,收盘后(≥16:05 ET)= 今日。
    晚班(21:00 PST)算的榜,次晨 06:45 班直接读同一份,零 GET。"""
    now = datetime.datetime.now(datetime.timezone(datetime.timedelta(hours=-4)))   # ET(夏令)近似,与 fetchers 同口径
...
def trend_board(today):
    ...
    if not universe:
        return [], 0, 0
    snaps = fetchers.history_snapshot(universe)
    board, rejected = [], 0
    min_score = int(os.getenv("TREND_MIN_SCORE", "5"))
    for sym in universe:
        d = snaps.get(sym)
        if not d or _floor_clause(d):
            ...
            continue
        pts, why, flags = 0, [], []
        up5, c5, c10 = d.get("up5"), d.get("chg5_pct"), d.get("chg10_pct")
        rsi, vx, h52, loc, chg = d.get("rsi14"), d.get("vol_x20"), d.get("high52_dist_pct"), d.get("close_loc"), d.get("chg_pct")
        if up5 is not None:
            if up5 >= 4: pts += 2; why.append("5日%d涨+2" % up5)
            elif up5 >= 3: pts += 1; why.append("5日%d涨+1" % up5)
        if c5 is not None:
            if 5.0 <= c5 <= 40.0: pts += 2; why.append("5日%+.1f%%+2" % c5)
            elif 2.0 < c5 < 5.0: pts += 1; why.append("5日%+.1f%%+1" % c5)
            elif c5 > 40.0: pts += 1; why.append("5日%+.1f%%+1" % c5); flags.append("追高风险")
        if c10 is not None and c10 >= 10.0: pts += 1; why.append("10日%+.1f%%+1" % c10)
        if rsi is not None:
            if 55 <= rsi <= 80: pts += 1; why.append("RSI%.0f+1" % rsi)
            elif rsi > 85: pts -= 1; why.append("RSI%.0f-1" % rsi); flags.append("过热")
        if vx is not None and vx >= 1.2: pts += 1; why.append("放量%.1fx+1" % vx)
        if h52 is not None and h52 >= -10: pts += 1; why.append("近52周高+1")
        if loc is not None and loc >= 0.6: pts += 1; why.append("收高位+1")
        if chg is not None and chg > 0: pts += 1; why.append("末根收涨+1")
        if pts >= min_score:
            board.append({"symbol": sym, "trend_score": pts, "trend_why": " ".join(why), "price": d.get("price"),
                          "chg5_pct": c5, "chg10_pct": c10, "up5": up5, "rsi14": rsi, "vol_x20": vx,
                          "adv20_musd": round((d.get("adv20_usd") or 0) / 1e6, 1), "flags": flags,
                          "asof": d.get("date") or key})
        else:
            rejected += 1
    ...
    try:
        ...
        json.dump({"key": key, "universe_n": len(universe), "rejected_n": rejected, "board": board},
...
def candidate_pool(cross):
    ...
    for s in universe:
        ...
        chg = d.get("chg_pct")
        pts, why, flags = 0, [], []
        if "异动" in tags:          # v3.26.3:上过涨幅榜且过了地板本身是一路信号(ASST 型纯异动票此前只靠涨幅分)
            pts += 1; why.append("异动+1")
        if "最活跃" in tags:
            if chg is not None and chg > 0:
                pts += 3; why.append("最活跃+3")
            else:
                pts += 1; why.append("最活跃(未收涨)+1"); flags.append("当日未收涨")
        if "趋势🔥" in tags:
            pts += 3; why.append("趋势榜+3")
        if "同频🔥" in tags:
            pts += 2; why.append("同频团+2")
        if "热簇🔥" in tags:
            pts += 1; why.append("热簇+1")
        if chg is not None:
            if 2.0 <= chg <= 20.0:
                pts += 2; why.append("日%+.1f%%+2" % chg)
            elif 0 < chg < 2.0:
                pts += 1; why.append("日%+.1f%%+1" % chg)
            elif chg > 20.0:
                pts += 2; why.append("日%+.1f%%+2" % chg); flags.append("追高风险")
            else:
                why.append("日%+.1f%%+0" % chg)
        if "连涨" in tags:
            pts += 1; why.append("连涨+1")
        h52 = d.get("high52_dist_pct")
        if h52 is not None and h52 >= -5:
            pts += 1; why.append("近52周高+1")
        loc = d.get("close_loc")
        if loc is not None and loc >= 0.7:
            pts += 1; why.append("收高位+1")
        vx = d.get("vol_x20")
        if vx is not None and vx >= 1.5:
            pts += 1; why.append("放量%.1fx+1" % vx)
        if "自选" in tags:
            pts += 1; why.append("自选+1")
        rsi = d.get("rsi14")
        if rsi is not None and rsi > 75:
            flags.append("RSI%.0f过热" % rsi)
        pool.append({"symbol": s, "score": pts, "score_why": " ".join(why), "eyes": sorted(tags),
                     "price": d.get("price"), "chg_pct": chg,
                     "adv20_musd": round((d.get("adv20_usd") or 0) / 1e6, 1),
                     "mcap_b": d.get("mcap_b"), "rsi14": rsi, "close_loc": loc,
                     "vol_x20": vx, "high52_dist_pct": h52, "flags": flags})
    # v3.26.3(戌 8-24 抓:PATH +1.77% 靠热簇一路信号挂在池尾):入池最低分 POOL_MIN_SCORE(默认 4,
    # 至少两路信号),低分票进筛除行带 score_why,不无痕
    min_score = int(os.getenv("POOL_MIN_SCORE", "4"))
...
def _leg_tier(row):
    ...
    pts, why = 0, []
    loc, chg, vx = row.get("close_loc"), row.get("chg_pct"), row.get("vol_x20")
    if loc is not None:
        if loc >= 0.7:
            pts += 2; why.append("收高位%.2f+2" % loc)
        elif loc < 0.3:
            pts -= 2; why.append("收低位%.2f-2" % loc)
    if chg is not None:
        if chg >= 1.0:
            pts += 1; why.append("日%+.1f%%+1" % chg)
        elif chg <= -1.0:
    ...
    if c5 is not None:
        if 2.0 <= c5 <= 15.0:
            pts += 1; why.append("5日%+.1f%%+1" % c5)
        elif c5 < -5.0:
            pts -= 1; why.append("5日%+.1f%%-1" % c5)
    if rsi is not None:
        if 50 <= rsi <= 70:
            pts += 1; why.append("RSI%.0f+1" % rsi)
        elif rsi > 75 or rsi < 40:
            pts -= 1; why.append("RSI%.0f-1" % rsi)
    if vx is not None and vx >= 1.3 and (chg or 0) > 0:
        pts += 1; why.append("放量%.1fx+1" % vx)
    if row["dk_risk"]:
        pts -= 1; why.append("双杀位-1")
    row["earn_score"], row["earn_why"] = pts, " ".join(why) or "无读数"
    return row
...
def _engine_card(cross):
    ...
    if "trend_board" in cross:   # v3.28:自算趋势榜(不是 FMP 榜)
        ...
        if tb:
            rows.append(("趋势榜(自算·全市场·5日,分[why])", " · ".join(
                "%s %d分[%s]%s" % (t["symbol"], t["trend_score"], t.get("trend_why", ""), ("⚠" + ",".join(t["flags"])) if t.get("flags") else "")
                for t in tb[:10])))
        else:
    ...
    if cross.get("amc_tonight"):
        def _amcfmt(x):
            ...
            es = x.get("earn_score")
            return x.get("symbol", "?") + ("⚠" if hot else "") + ("(%+d:%s)" % (es, x.get("earn_why", "")) if es is not None else "")
        rows.append(("今晚财报(AMC,按尾盘形态分,⚠=双杀风险位)",
    ...
    if cross.get("bmo_tomorrow"):
        ...
        rows.append(("明日财报(BMO 伏击名单,按尾盘形态分,⚠=双杀风险位)",
...
def render_brief_html(date, mode, data, raw_text, cross=None):
    ...
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            ...
            '%s<footer>build scout v3.28 · 自算趋势榜+形态闸+候选池+地板 · 三班制+BMO伏击+FMP路由 · FMP主源+Theta+Alpaca backup · emit/tape_check/rsi14_tape/源清单lint/EXPANDED-GLM · 影子lane · 账本卫生 · Grid降噪 · 四槽卡 · ET锚定 · '
...
def gate_static_code() -> None:
    ...
    if "def fmp_board_syms" in sa and "不在当日榜" in sa and "FMP榜+2" in sa and os.path.exists(os.path.join(HERE, "acceptance_fmp_board.py")):
        _ok("v3.27.1 财报 call 腿必须在当日榜(资金确认)+ 名单榜加分 + 现场验收脚本在场")
    else:
        _fail("v3.27.1 榜硬门/验收脚本缺失(Lyra:不在 top mover 榜的不收)")
    if ("def trend_board(" in sa and '"趋势🔥"' in sa and "趋势榜+3" in sa and "def fetch_screener_universe" in fe
...
print("✅ 今晨已重渲,卡片已回:刷新页面即可" if _ok() else
      "❌ 未恢复,自诊(原样带来,勿自行分析):\n" + "\n".join((r.stdout + r.stderr).strip().splitlines()[-6:]))
SELFCHECK
echo ""; echo "✓ Scout Agent v3.28.0: $(pwd) — 首跑 python3 scout_agent.py --mode morning;门禁 python3 verify_scout_deploy.py"
