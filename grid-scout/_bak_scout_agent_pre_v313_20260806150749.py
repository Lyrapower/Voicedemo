#!/usr/bin/env python3
"""scout_agent.py —— Scout Agent v3.8(守恒交付语义 + 本机接线累加)

版本纪律:禁跑 install.sh 整包覆盖本机接线(.env/expanded/aether/ssl/FRED回退)。
v3.8 明确优化处**替代**旧逻辑(防矛盾):
  · 宇宙三层取代「仅 SP500∪Nasdaq」硬闸:A主池 / B事件驱动过流动性闸 / C海外ADR巨头
  · 财报两步手册 + 爬虫#10 earnings_calendar;strategy.earnings_note + candidate.liquidity 必填
  · 复盘回路:晚班 review.json → 次晨战绩+滚动5日命中率;RSI14 时机过滤
  · 引擎 results 键(已修)保留
保留本机非冲突累加:
  · SCOUT_REVIEW=expanded(驳回 Aster);ensure_hedge/三灯地板;rank 1/2/3+rank_reason
...
def _engine_earnings_cards(raw, limit=2, exclude: set[str] | None = None) -> list:
    ...
    for sym, items in idx.items():
        ...
        day_delta = 0
        try:
            day_delta = (datetime.date.fromisoformat(day) - datetime.date.today()).days
        except ValueError:
            day_delta = 99
...
def _has_earnings_candidate(cands, raw) -> bool:
    ...
    for c in cands:
        ...
        for e in (c or {}).get("evidence") or []:
            if isinstance(e, dict) and str(e.get("source") or "").lower() in (
...
def ensure_candidates(data, raw=None, *, min_n=None):
    ...
    # 财报硬优先:日历有当日/近窗大中盘,但主菜零财报名 → 引擎插入至少1张(可顶掉末位非财报)
    ...
    if len(cands) >= need:
        ...
        data["no_candidate_reason"] = (
            ...
            "禁止无流动性微盘凑数;对冲腿另见 hedge" % (len(cands), need)
        )
...
def _cand_card(c, tag, badge="SCOUT"):
    ...
    # 字段序对齐 brief sample v38:形态→…→作废→财报→流动性→T+0→关键位
    rows += [("形态", st.get("type")), ("行权价逻辑", st.get("strike_logic")),
             ...
             ("财报", st.get("earnings_note")), ("流动性", c.get("liquidity")),
...
def render_brief_html(date, mode, data, raw_text, cross=None, *, review_origin=""):
    ...
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            ...
            '%s<footer>build scout v3.8 · 三层宇宙 · 财报手册 · 复盘回路 · 无证据不点名 · 合成 GEX 不作数 · '
