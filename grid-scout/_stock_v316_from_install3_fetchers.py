...
def et_now_hm():
    """当前 ET 钟点 HH:MM(prompt/渲染共用,杜绝硬编码 9:45);
    ZoneInfo 不可用回退本机钟点并标注(仅 PST 机器语义近似)。"""
    if _ET is None:
...
def fetch_edgar_recent():
    ...
    def go():
        base = trading_date()      # v3.13(守恒):检索窗随交易日锚定,UTC 机器晚班不漂"明天"(时间语义同族)
...
def fetch_commodities():
    """WTI/GOLD 经 ETF 代理(USO/GLD)。stooq 退役 2026-08-06(被墙)。"""
    def go():
        out = [_daily(s, n, "commodities") for s, n in (("cl.f", "WTI"), ("xauusd", "GOLD"))]
...
ALPACA_FEED = os.getenv("ALPACA_FEED", "").strip()   # 按账户实况填 iex/sip,不预设


def _alp(path):
    h = {"APCA-API-KEY-ID": os.getenv("ALPACA_KEY_ID", ""),
         "APCA-API-SECRET-KEY": os.getenv("ALPACA_SECRET_KEY", "")}
    sep = "&" if "?" in path else "?"
    url = ALPACA_BASE + path + ((sep + "feed=" + ALPACA_FEED) if ALPACA_FEED else "")
    req = urllib.request.Request(url, headers=h)
    with urllib.request.urlopen(req, timeout=15) as r:
        return json.loads(r.read().decode())


# 指数/商品 → ETF 代理(Lyra 拍板 2026-08-06,stooq 被墙全线退役;被墙源不留兜底:
# 20s 超时×串行=拖班炸弹)。字段名不变(SP500/VIX/WTI/GOLD),引擎/prompt/渲染零改动。
PROXY = {"^SPX": "SPY", "^NDQ": "QQQ", "^VIX": "VIXY", "CL.F": "USO", "XAUUSD": "GLD"}


def _daily(sym, name, skip_src):
    """日线读数(Alpaca dailyBar,盘中即时更新=真盘初 tape)。输出与旧版同构
    + proxy/asof 溯源;失败响亮入 skip,无兜底。"""
    try:
        t = sym.upper()
        t = t[:-3] if t.endswith(".US") else t
        t = PROXY.get(t, t)
        snap = _alp("/v2/stocks/snapshots?symbols=" + t).get(t) or {}
        db, pb = snap.get("dailyBar") or {}, snap.get("prevDailyBar") or {}
        if not db or not pb:
            _log_skip(skip_src, t, "no daily/prev bar")
            return None
        o, h, l, c, pc = db["o"], db["h"], db["l"], db["c"], pb["c"]
        bars = (_alp("/v2/stocks/bars?symbols=%s&timeframe=1Day&limit=60" % t)
                .get("bars") or {}).get(t) or []
        closes = [b["c"] for b in bars] or [pc, c]
        d = {"name": name, "proxy": t, "date": (db.get("t") or "")[:10], "asof": db.get("t"),
             "open": o, "high": h, "low": l, "close": c,
             "chg_pct": round((c / pc - 1) * 100, 2), "rsi14": _rsi14(closes),
...
def fetch_indices():
    """爬虫#3:指数日线 + tape 读数——经 ETF 代理 SPY/QQQ/VIXY(Alpaca;stooq 退役)。"""
    def go():
        out = [_daily(sym, name, "indices")
...
def fetch_hedge_assets():
    """爬虫#8:对冲资产日线(GLD/SLV/OXY/USO,Alpaca)——
    ...
    def go():
        ...
        for sym, name, cls in spec:
            d = _daily(sym, name, "hedge_assets")
            if d:
                d["cls"] = cls
                out.append(d)
        return out
...
def fetch_sectors():
    """爬虫#12:板块 tape(SPDR 11 板块 + SMH,Alpaca 同源)——
    ...
    def go():
        ...
        out = [_daily(sym, name, "sectors") for sym, name in spec]
        return [x for x in out if x]
