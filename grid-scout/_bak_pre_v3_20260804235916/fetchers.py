...
def _ssl_ctx():
    """Framework Python 缺系统 CA 时用 certifi(否则全源 SSL 失败)。"""
    try:
        import certifi
...
def fetch_edgar_recent():
    """EDGAR 全文检索:近两日 8-K/425 并购信号。官方,需 UA。
       # [守恒patch 2026-07-30 edgar] URL 清理 + size=50 + startdt/enddt 正确格式"""
    def go():
        today = datetime.date.today().isoformat()
        frm = (datetime.date.today() - datetime.timedelta(days=2)).isoformat()
        url = ("https://efts.sec.gov/LATEST/search-index?q=%22merger%22"
               "&forms=8-K&startdt=" + frm + "&enddt=" + today + "&size=50")
        data = json.loads(_get(url))
        hits = data.get("hits", {}).get("hits", [])
        return [{"company": h["_source"].get("display_names", ["?"])[0],
                 "form": h["_source"].get("file_type"),
                 "filed": h["_source"].get("file_date")} for h in hits]
    ...
    def go():
        ...
        for mkt in data:
            try:
                ...
                # [守恒patch 2026-07-30 polymarket] json.loads 挂了也 continue,不放 raise 出锅
                prices = mkt.get("outcomePrices")
                if isinstance(prices, str):
                    try:
                        prices = json.loads(prices)
                    except Exception:
                        _log_skip("polymarket", mkt.get("question", "?"), "outcomePrices json parse failed")
                        continue
