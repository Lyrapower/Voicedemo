...
def _fmp_base():
    return os.getenv("FMP_BASE", "https://financialmodelingprep.com/stable").rstrip("/")
...
_FMP_RPM_PEAK = [0]        # 本进程共享 key rpm 峰值
...
def _fmp_pace():
    ...
    while True:
        ...
        if len(_FMP_CALL_TS) < limit:
            ...
            _FMP_RPM_PEAK[0] = max(_FMP_RPM_PEAK[0], len(_FMP_CALL_TS))
            return
...
def _fmp_tick():
    ...
    try:
        ...
        pass
    budget = int(os.getenv("FMP_DAILY_BUDGET", "-1"))  # -1 = 记账不设警告线
    if budget > 0:
...
def _fmp_call_routed(kind, sym, url_fn):
    ...
    fams = ([fam] if fam else []) + [f for f in ("stable", "legacy") if f != fam]
    errs = []
    for f in fams:
        try:
...
def _fmp_quote(symbols):
    ...
    if st:
        ...
        if fam == "legacy" and _FMP_BATCH_OK and len(st) > 1:
            try:
...
def fetch_screener_universe(cache_path=None, ttl_s=20 * 3600):
    ...
    定义:非 ETF/基金、在交易、NYSE/NASDAQ/AMEX、price≥POOL_PRICE_FLOOR。
    无 price×volume 粗筛;精地板 = history adv20≥$50M(与 build_universe 同:20 日 close×v,剔最新一根)。"""
    ...
    try:
        ...
        json.dump({"ts": time.time(), "n": len(syms), "raw_n": len(rows), "volume0_passed": 0, "symbols": syms},
                  open(cache_path, "w", encoding="utf-8"))
        print("[fetchers] screener 宇宙 %d/%d(无 price×volume 粗筛,adv20 精地板后定)" % (len(syms), len(rows)))
    except Exception:
        pass
    return syms
...
def data_plane_banner():
    ...
    if _fmp_key():
        return ("FMP 主源(当日已用 %d · rpm 峰值 %d)· Theta 第二源=%s · Alpaca backup=%s · %s · stooq 已移除"
                % (fmp_calls_today(), fmp_rpm_peak(), "在线" if _theta_up() else "未起(跳过)", alp, feed_ah_label()))
    if ak and asec:
...
def fetch_most_active():
    ...
    def go():
        ...
        urls = [_FMP_STABLE_BASE + "/most-actives", _fmp_base() + "/actives",
...
def fetch_market_movers():
    ...
    def go():
        ...
        cands = {
            "gainers": [_FMP_STABLE_BASE + "/biggest-gainers",
                        _fmp_base() + "/stock_market/gainers"],
            "losers": [_FMP_STABLE_BASE + "/biggest-losers",
                       _fmp_base() + "/stock_market/losers"],
        }
        route = _fmp_route_load()
        out = []
...
def run_all():
    ...
    res = [f() for f in ALL]
    if _fmp_key():
        print("[fetchers] FMP 当日用量 %d(记账不设闸) · rpm 峰值 %d" % (fmp_calls_today(), fmp_rpm_peak()))
    return res
