...
候选铁律:每个候选必须锚定隔夜采集中的具体条目,逐条给出处;禁止凭训练记忆点名。
...
S2 财报前置(Lyra 2026-08-07 定;本槽只做财报"前"侧,两腿二选一):
   a) 提前 run-up:earnings_calendar 未来 2-8 个交易日财报的票,吃预期消化上涨段,
      公布前必须离场(手册第二步)
   b) 当日 AMC 尾盘伏击:amc_tonight 名单择一,~12:50 PST 前买入,持过财报,
      次日盘初按结果处置(gap-and-go 续持 / 开盘即走)——本系统唯一合法持仓过夜
      例外;earnings_note 必须写"持过财报,IV crush 风险自担"
   双杀排除铁律(AMD/NVDA 型大热禁入):amc_tonight 实测 chg5_pct>15% 或 rsi14>75
      = 利好已定价、多空双杀高发,机械禁入;AI 巨头共识热票(AMD/NVDA/TSLA 级)禁入;
      选预期温和、IV 未过热的中大盘
   已出结果的财报票(今晨 BMO/昨夜 AMC gap)不属 S2——由 movers 硬闸强制显式处理,
      够强则进 S1/S4
S3 事件:8-K 重大文件(EX-2.x 并购/资产/重大合同)或 FDA approval——
   必须过流动性实测闸(liquidity_check 过闸);无够格标的宁可空槽,禁硬凑小票
S4 引擎位:由跨资产引擎读数驱动——领涨板块龙头、迁徙腿(divergence 证实)或
...
时间窗铁律:每卡 entry_window_pst / exit_window_pst 必填,给具体 PST 时段
(如 "7:15-7:45 突破确认入场" / "11:30 未达标离场,12:50 硬平仓"),禁"视情况"。
量能铁律:入场条件里凡"放量"必须引用实测 vol_x20(≥1.5 才算放量;无读数写明"量能无实测");
隔夜/日内铁律:index_session_split 直面 T+0 结构——日内 20 日分量为负的体制,
S1/S4 日内做多降杠杆表述并写明;隔夜分量显著为正时 S2(b) 伏击腿(唯一吃隔夜的腿)
排名可升,rank_reason 必须引用该读数;
风险预警铁律:risk_alerts 非空必须逐条回应,写进大盘 logic 或 hedge.basis,禁无视。
...
- 名单内标的走 S2(b) 尾盘伏击腿(~12:50 PST 买入持过财报)或不做;
  除该腿外禁持仓过财报;双杀排除铁律先于一切(chg5>15% 或 rsi>75 机械禁入)
...
5. 风险雷达(黑天鹅/机构拉高出货/全线下跌排查,永不留空):逐项核对——
   ...
   今晚 AMC:按 amc_tonight 全名单逐个点名(点名依据=名单本身,不做侦测);
...
6. 晨会复盘与明日调优(复盘读数必须逐腿引用,方向命中口径=收盘对昨收):
   ...
   入场条件怎么改、RSI 时机过滤怎么用;明日/本周财报名单点名,标注 run-up 机会
...
def _load_env_file():
    """读 scout 目录 .env(KEY=VAL 行)进环境,不覆盖已有值——launchd 班次无需
    改 plist 即可拿到 SHADOW_*/DS_MAX_TOKENS 等新变量(plist 保护规矩的配套件)。"""
...
def shadow_call(prompt):
    ...
    try:
        r = _http((os.getenv("SHADOW_BASE") or DS_BASE) + "/chat/completions",
                  {"model": model, "max_tokens": int(os.getenv("DS_MAX_TOKENS", "6000")),
                   "messages": [{"role": "user", "content": prompt}]},
                  {"Authorization": "Bearer " + (os.getenv("SHADOW_KEY") or DS_KEY or "")})
...
def ds_call(prompt):
    ...
    r = _http(DS_BASE + "/chat/completions",
              {"model": DS_MODEL, "max_tokens": int(os.getenv("DS_MAX_TOKENS", "6000")),
               "messages": [{"role": "user", "content": prompt}]},
...
def build_review(date, suffix=""):
    ...
    mp = os.path.join(OUT, "briefs", "%s-morning%s.json" % (date, suffix))
    if not os.path.exists(mp):
        return None
    doc = json.load(open(mp, encoding="utf-8"))
    ds = doc.get("ds") or doc
    legs = ([dict(l, _src="candidate") for l in ds.get("candidates") or []] +
            [dict(l, _src="hedge") for l in (ds.get("hedge") or {}).get("legs") or []])
    out = []
    for l in legs:
    ...
    rev = {"date": date, "legs": out, "rejected_review": rej_out,
           ...
           "basis": "收盘对昨收判定方向命中(仅候选腿计入;对冲腿分账);非盘中入场路径复现"}
...
def render_brief_html(date, mode, data, raw_text, cross=None):
    ...
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            ...
            '%s<footer>build scout v3.23 · 影子lane · 账本卫生 · Grid降噪 · 隔夜日内 · 量能 · 四槽卡 · 盘后实测 · cap四层 · ET锚定 · 三层宇宙 · 复盘回路 · 无证据不点名 · '
            '渲染:_render_structured/_cand_card/_md_fallback</footer></body></html>'
            % (mode.upper(), _esc(date), BRIEF_CSS, mode, _esc(date),
               "盘初 ~9:45 ET(当日行为部分K线)" if mode == "morning" else "收盘(全日K线)", body))
...
def main():
    ...
    a = ap.parse_args()
    _load_env_file()   # v3.23:plist 保护的配套件——新变量走 .env,无需改 plist
    ...
    if a.mode == "morning":
        ...
        prompt = build_trading_prompt(payload, ws, yday, cross, prev)
        body = ds_call(prompt)
        data = _extract_json(body)
        if data is None:
            hint = "疑输出截断,查 DS_MAX_TOKENS" if body.lstrip().startswith("{") else "DS 未按 schema"
            print("[scout] 晨会单 JSON 解析失败(%s)——降级为文本分节渲染(响亮记录)" % hint)
        else:
            data = apply_liquidity_gate(data)
        render_console("Scout 晨会交易任务单 · " + today, body, today, "morning", data, cross)
        sbody = shadow_call(prompt)
        ...
        body = denoise(ds_call(build_evening_prompt(payload, yday, cross, rev)))
