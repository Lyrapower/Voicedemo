"""fetchers.py · 整合 v1(2026-08-07:GLM 加固版为基 + 守恒四点 afterhours 合并 + EDGAR 交易日锚定;主 tape 路径按现网血统维持 stooq,未夹带数据源改动) —— 晚报确定性采集层(零 LLM,零爬虫,全官方/公开源)。
    ...
    def go():
        ...
        for sym, name in (("cl.f", "WTI"), ("xauusd", "GOLD")):
            try:
                ...
                if len(rows) >= 2:
                    head = rows[0].split(",")
                    last = rows[-1].strip().split(",")  # 防末行空字符
                    if len(head) == len(last):
                        out.append({"name": name, **dict(zip(head, last))})
                    else:
                        _log_skip("commodities", sym, "csv column mismatch")
                else:
