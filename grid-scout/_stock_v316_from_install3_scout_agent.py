...
v3.15 = 两窗合流;v3.16(2026-08-06)= 规格七件全落码:晚班已出叙事/§6 排今日/晚班ET钟/
复盘watch段/tape_check盖章/RSI禁自估/amc带读数 + stooq 全线退役(被墙)→ Alpaca 主源
+ 指数商品 ETF 代理(SPY/QQQ/VIXY/USO/GLD,字段名不变引擎零改)。
"""
...
- RSI14 是时机过滤器不是方向信号:凡采集读数中有 rsi14 者(指数/对冲/movers/amc 名单),
  >70 禁追高须写回踩确认、<30 禁追空;读数里没有的标的禁止自估 RSI——
...
5. 风险雷达(黑天鹅/机构拉高出货/全线下跌排查,永不留空):逐项核对——
   ...
   (gap-and-go / 回补过半 fade 判据);名单自带收盘读数,盘后价若采集里没有
   就明写"盘后读数缺失",禁编数字(点名依据=名单本身,不做"谁出了结果"的侦测);
...
6. 晨会复盘与明日调优(复盘读数必须逐腿引用,方向命中口径=收盘对昨收):
   ...
   RSI 时机过滤怎么用;明日及以后(date>今日)的财报名单从 earnings_calendar 点名,标注 run-up 机会;
   今日已出结果者不属 run-up,禁入此节(其复盘走 watch 段与风险雷达);
   watch 段(雷达名单收盘读数)逐个讲清,尤其 |chg|≥8% 者——"看见了但没入选"
   的复盘,此后 TEAM 类必须在此留痕,空缺即故障
...
def build_review(date):
    ...
    for l in legs:
        ...
        if not re.fullmatch(r"[A-Z]{1,5}", t or ""):
            ...
            continue
        d = fetchers._daily(t.lower() + ".us", t, "review")
        if not d:
            continue
    ...
    amc_syms = [(x.get("symbol") if isinstance(x, dict) else x) for x in (eng.get("amc_tonight") or [])]
    for sym in amc_syms[:8] + [m.get("symbol") for m in (eng.get("earnings_movers") or [])[:3]]:
        if not sym or sym in seen or not re.fullmatch(r"[A-Z]{1,5}", sym):
            continue
        seen.add(sym)
        d = fetchers._daily(sym.lower() + ".us", sym, "review_watch")
        if d:
...
def earnings_movers(payload, today):
    ...
    # 30 次取数(Alpaca)是上限兜底,榜单仍只出前 8
    for sym, tag in syms[:30]:
        r = fetchers._daily(sym.lower() + ".us", sym, "earnings_movers")
...
def amc_tonight(payload, today):
    ...
    显式喂 DS 供手册第二步(run-up)评估,并上引擎卡。名单保持日历的市值序,取前15
    ("名单前三"即列表前三项)。"""
    out = []
    for src in payload.get("results", payload.get("sources", [])):
        if src.get("source") == "earnings_calendar":
            for it in src.get("items", []):
                ...
                if it.get("date") == today and "after" in (it.get("when") or "") \
                    ...
                    out.append(sym)
    # "已出结果者"的判定 = 名单本身;禁加侦测机制(侦测=编,两窗共定)。
    out2 = []
    for sym in out[:15]:
        d = fetchers._daily(sym.lower() + ".us", sym, "amc_tonight")
        out2.append({"symbol": sym, "chg_pct": (d or {}).get("chg_pct"),
                     "rsi14": (d or {}).get("rsi14"), "close_loc": (d or {}).get("close_loc")})
    return out2
...
def apply_liquidity_gate(data):
    ...
    for l in legs:   # v3.16 ④:RSI/tape 逐腿实测盖章——治"铁律无供数"(RSI 事故);只盖章不删卡
        td = fetchers._daily((l.get("ticker") or "").lower() + ".us",
                             (l.get("ticker") or "").upper(), "tape_check")
        if td:
            rsi, dirn = td.get("rsi14"), (l.get("direction") or "call").lower()
            v = ("违规:RSI%.0f 超买,追高需回踩确认" % rsi) if (rsi and rsi > 70 and dirn == "call")                 else (("违规:RSI%.0f 超卖,禁追空" % rsi) if (rsi and rsi < 30 and dirn == "put") else "过")
...
def _cand_card(c, tag, badge="SCOUT"):
    ...
    rows = [("形态", st.get("type")), ("行权价逻辑", st.get("strike_logic")),
            ...
            ("流动性实测(引擎)", lc_txt),
            ("RSI/tape 实测(引擎)", (lambda tc: ("RSI %s · %+.2f%% · %s" % (tc.get("rsi14"), tc.get("chg_pct") or 0, tc.get("verdict"))) if tc else None)(c.get("tape_check"))),
...
def _engine_card(cross):
    ...
    if cross.get("amc_tonight"):
        rows.append(("今晚财报(AMC watch)",
                     " · ".join(("%s %+.1f%%" % (x["symbol"], x["chg_pct"])) if isinstance(x, dict) and x.get("chg_pct") is not None
                                else (x["symbol"] if isinstance(x, dict) else str(x))
                                for x in cross["amc_tonight"][:6])))
...
def render_brief_html(date, mode, data, raw_text, cross=None):
    ...
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            ...
            '%s<footer>build scout v3.16 · DS 决策官 JSON 结构化 · 无证据不点名 · 合成 GEX 不作数 · '
...
def main():
    ...
    a = ap.parse_args()
    src = "alpaca(feed=%s)" % (fetchers.ALPACA_FEED or "账户默认") if os.getenv("ALPACA_KEY_ID")           else "!! ALPACA_KEY_ID 未配置——个股/代理读数将全线响亮失败(stooq 已退役,无兜底)"
    print("[scout] 数据源:", src)
