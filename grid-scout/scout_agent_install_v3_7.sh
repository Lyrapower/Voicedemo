#!/usr/bin/env bash
# ============================================================================
# Scout Agent · v3.7 · 一键安装(守恒亲手交付;覆盖 v3.6,完整替代非 patch)
#   v3.7(Lyra 反转 D2,2026-08-05):晨班回 6:45 PST——开盘后15分钟盘初 tape
#   信息量大于盘前;prompt 措辞改"盘初"并强制标注部分K线读数口径,时刻服从信息量
...
#   ②晨班时刻史:v3.2 曾改 6:00,v3.7 按 Lyra 反转拍板回 6:45 PST(=9:45 ET)——
#     盘初 15 分钟 tape 信息量 > 盘前,时刻不动、prompt 措辞改"盘初"并标注部分K线口径
...
#   双班:morning 6:45(盘初)交易任务单(workstation 联动)/ evening 9pm 复盘+弹药
#   bash scout_agent_install_v3_7.sh [目标目录]   默认 ./grid-scout
#   首跑三步见 README.md
# ============================================================================
set -euo pipefail
ROOT="${1:-./grid-scout}"
...
<?xml version="1.0" encoding="UTF-8"?>
<!-- 晨报 6:45am PST(=9:45 ET,开盘后15分钟——Lyra 反转拍板 2026-08-05:盘初 tape 信息量 > 盘前,时刻不动改措辞);注册: cp 到 ~/Library/LaunchAgents 后 launchctl load -->
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
