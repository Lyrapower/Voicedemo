...
def fetch_earnings_calendar():
    """爬虫#10:Nasdaq 财报日历(今起5个自然日,零 key)——v3.8 财报两步手册数据前提。"""
    def go():
        out = []
        base = datetime.date.today()
        for k in range(5):
            day = (base + datetime.timedelta(days=k)).isoformat()
            try:
                raw = json.loads(_get(
                ...
                for r in sorted(rows, key=cap, reverse=True)[:15]:
                    out.append({
                        "symbol": r.get("symbol"),
...
# 十爬虫(v3.8 +#10 earnings_calendar);本机保留 ssl/FRED 回退/Alpaca 优先
ALL = [fetch_treasury_yields, fetch_fred, fetch_indices, fetch_edgar_recent,
       ...
       fetch_fear_greed, fetch_earnings_calendar]
