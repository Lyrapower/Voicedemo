...
def _engine_card(cross):
    ...
    if cross.get("amc_tonight"):
        rows.append(("今晚财报(AMC watch)", " · ".join(cross["amc_tonight"][:6])))
...
def render_brief_html(date, mode, data, raw_text, cross=None):
    ...
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            ...
            '%s<footer>build scout v3.15 · 盘后实测 · cap四层 · ET锚定 · 三层宇宙 · 复盘回路 · 无证据不点名 · '
...
def main():
    ...
    if a.mode == "morning":
        ...
        if cross["amc_tonight"]:
            ah = fetchers.fetch_afterhours(cross["amc_tonight"])
