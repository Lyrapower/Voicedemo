...
隔夜采集:{json.dumps(raw, ensure_ascii=False)[:5500]}
工作站读数(GEX/特征):{json.dumps(ws, ensure_ascii=False) if ws else "工作站无可用实弹数据(不可达或仅合成数据),基于隔夜数据判断"}
昨日采集(对比用):{json.dumps(yday, ensure_ascii=False)[:1800] if yday else "无"}
...
用 markdown,严格以 ## 分节(要闻催化/赔率变化/明日日历/明日方向),
...
def expanded_review(mode: str, ds_draft: str, data, indices_snap) -> dict:
    ...
    prompt = build_review_prompt(
        ...
        data,
        indices_snap,
        reviewer="Grid EXPANDED 大底座 · GLM substrate + Aster 校验接纳",
    )
    ...
    body = {
        ...
        "assets": [],
        "client_context": "scout_morning_review · no chat history · A/B vs glm_direct",
        "cloud_enabled": True,
    }
...
def _cand_card(c, tag):
    ...
    return (
        '<div class="card"><span class="badge">SCOUT %s</span>'
...
def _render_structured(date, data, indices_snap=None):
    ...
    out = [
        ...
        "<span>VIX <b class=\"num\">%s</b></span><span>置信 <b>%s</b></span></div>"
        % (
            _esc(date),
    ...
    if gaps:
        out.append(
            '<section><h2>三 · 数据缺失与矛盾标注</h2><div class="card"><table>'
    ...
    if data.get("conclusion"):
        out.append(
            '<section><h2>四 · 结论</h2><div class="card">%s</div></section>'
...
def render_brief_html(date, mode, data, raw_text, indices_snap=None):
    ...
    return (
        ...
        "%s<footer>build scout v3.3 · DS JSON → review(GLM|EXPANDED) · indices(^vix) → aether OPTION · "
...
def emit_aether(
    ...
    *,
    ...
    payload = {
        ...
        "via": "scout_v3_3_ds_expanded_ab",
        "brief_path": bp,
        "workstation": ws,
        "console_ds_task": ds_task_id,
        "console_glm_task": glm_task_id,
        "pipeline": "DS JSON→GLM直连|b11 EXPANDED review编译",
...
def main():
    ap = argparse.ArgumentParser(
        description="Scout Agent v3.3(DS JSON→GLM|b11 EXPANDED)"
    )
    ...
    ap.add_argument(
        ...
        default=os.getenv("SCOUT_REVIEW", "both"),
        help="review/编译路径:glm=8501直连 · expanded=b11 STUDIO·EXPANDED · both=A/B",
    )
    a = ap.parse_args()
