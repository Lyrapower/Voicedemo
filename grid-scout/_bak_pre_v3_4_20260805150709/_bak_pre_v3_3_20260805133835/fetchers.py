...
def fetch_indices():
    """SP500/Nasdaq/VIX 指数日线 via stooq(与 commodities 同端点,零 key)。
    v3.2 池化前提:大盘方向以指数计,不再以 SPY/QQQ 两 ETF 代指。
    符号若与 stooq 实况不符会逐条响亮入 skip,不静默。"""
    def go():
        out = []
        for sym, name in (("^spx", "SP500"), ("^ndq", "NASDAQ"), ("^vix", "VIX")):
            try:
                ...
                if len(rows) >= 2:
                    ...
                    _log_skip("indices", sym, "empty csv")
            except Exception as e:
