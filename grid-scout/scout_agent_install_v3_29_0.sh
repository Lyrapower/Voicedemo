#!/usr/bin/env bash
# ============================================================================
# Scout Agent · v3.29.0 · 总包(完整替代;plist/verify 保护;自检自跑;落地后自归档一份进仓)
...
# Scout Agent v3.29.0(DS 决策官 · 三班制 + 六因子核 + 准入闸 + 自算趋势榜 + 仓位档/Kelly)
...
def f5_batch(snaps, symbols):
    """F5:对一批候选取 Theta ATM call 的 gamma/|theta|,批内百分位。盘外/Theta 缺 → 全 None(因子缺,不装数)。
    返回 (sub_by_sym, raw_by_sym)。调用上限由调用方控(候选池 ≤ 40 票)。"""
    raws = {}
    for s_ in symbols:
        d = snaps.get(s_) or {}
        g = fetchers.theta_atm_call_greeks(s_, d.get("price"))
        raws[s_] = factors.f5_options_ratio(g)
    return factors.f5_options_batch(raws), raws
...
def candidate_pool(cross):
    ...
    for s in universe:
        ...
        if info.get("is_etf") is False and d and not _floor_clause(d):
            survivors.append(s)
    f5_sub, f5_raw = (f5_batch(snaps, survivors[:40]) if survivors else ({}, {}))
    for s in universe:
        d = snaps.get(s)
        tags = eyes[s]
        info = ind.get(s) if isinstance(ind.get(s), dict) else {}
...
cp -f "$SELF_PATH" "./scout_agent_install_v3_29_0.sh" 2>/dev/null && echo "[install] 安装脚本已归档 ./scout_agent_install_v3_29_0.sh" || true
echo ""; echo "✓ Scout Agent v3.29.0: $(pwd) — 首跑 python3 scout_agent.py --mode morning;门禁 python3 verify_scout_deploy.py"
