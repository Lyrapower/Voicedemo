#!/usr/bin/env bash
# ============================================================================
# Scout Agent · v3.4 · 一键安装(守恒亲手交付;覆盖 v3.3)
...
#   bash scout_agent_install_v3_4.sh [目标目录]   默认 ./grid-scout
#   首跑三步见 README.md
# ============================================================================
set -euo pipefail
ROOT="${1:-./grid-scout}"
...
对冲资产 GLD·SLV·OXY·USO/Fear&Greed。
晨会 hedge 段与晚报风险雷达永不空白:拉高出货(指数冲高回落+贪婪极值+对冲资产异动)
或黑天鹅出尽 → 必须给对冲腿评估;风险低也要写明依据。
PKG_EOF_000
...
def fetch_hedge_assets():
    ...
    def go():
        out = [_stooq_daily(sym, name, "hedge_assets")
               for sym, name in (("gld.us", "GLD"), ("slv.us", "SLV"),
                                 ("oxy.us", "OXY"), ("uso.us", "USO"))]
        return [x for x in out if x]
...
def fetch_polymarket():
    ...
    _EVENT_RX = re.compile(
        ...
        r"|etf|shutdown|default|banks?|nuclear|strikes?|wars?|invasion)\b", re.I)
    def go():
