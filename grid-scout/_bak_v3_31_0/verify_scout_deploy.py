...
def gate_static_code() -> None:
    ...
    if (
        "def glm_morning_land_or_die" in sa
        and "禁止把 DS 直出当落盘" in sa
        and "glm_morning_land_or_die(today)" in sa
    ):
        _ok("晨报 GLM land 硬闸(DS json 半成品,final 必须 glm52_cloud)")
    else:
        _fail("晨报 GLM land 硬闸缺失(Cursor 又可把 DS 直出当落盘)")
    ...
    if (fx and "FACTOR_BUDGET" in fx and "def score(" in fx and "import factors" in sa and "factors.score(" in sa
        ...
        if "禁止把 DS 直出当晨报落盘" in landt and 'origin != "glm52_cloud"' in landt and "(glm empty — ds json landed)" not in landt:
            _ok("晨报 land:GLM 空/非 glm52_cloud 禁止写 final+emit")
        else:
            _fail("scout_land_option 又可 GLM 空仍落盘(DS 直出假完成)")
    else:
