...
def _load_env_file():
    ...
    与下方常量——CONSOLE_KEY/OUT 均为 import 期快照,晚于此处加载的
...
OFFICER = "glm52"  # REMOVED DeepSeek 2026-09-03 · Scout 决策官改 8501 cloud_chat
...
# 公开 html / emit / *-final.md 必须 GLM 编译章。DS 只许半成品 (.json/.md/.ds.html)。
GLM_LAND_SHIFTS = ("morning", "midday", "earnings")
_SHIFT_HTML_LABEL = {"morning": "晨报", "midday": "午报", "earnings": "财报班", "evening": "晚报"}
_SHIFT_CLOCK = {
...
def shadow_call(prompt):
    ...
    if not model:
        return None
    try:
        r = _http((os.getenv("SHADOW_BASE") or GW) + "/task/cloud_chat",
                  {"substrate": model, "max_tokens": int(os.getenv("GLM_OFFICER_MAX_TOKENS", "12000")),
                   "persist": False,
...
def glm_officer_call(prompt):
    """Scout 决策官 · 8501 /task/cloud_chat substrate=glm52 · persist=False(不写 cloud-glm52)。"""
    payload = {
        "substrate": OFFICER,
        "persist": False,
        "memory_sealed": True,
        "max_tokens": int(os.getenv("GLM_OFFICER_MAX_TOKENS", "12000")),
        "messages": [{"role": "user", "content": prompt}],
    }
    import time as _t, socket as _sock
    last_err = None
    to = int(os.getenv("GLM_OFFICER_TIMEOUT", "420"))
    for attempt in range(int(os.getenv("SCOUT_OFFICER_ATTEMPTS", "5"))):
        try:
            r = _http(GW + "/task/cloud_chat", payload, timeout=to)
            content = (r.get("content") or r.get("choices", [{}])[0].get("message", {}).get("content", "") or "")
            if content.strip():
                return content
            print("[scout] glm52 空内容(第 %d 次),继续重试" % (attempt + 1))
        except Exception as e:
            last_err = e
            code = getattr(e, "code", None)
            if code and 500 <= code < 600 and attempt < 2:
                print("[scout] glm52 HTTP %d,退避重试(%d/3)" % (code, attempt + 1))
            ...
            if is_to and attempt == 0:
                print("[scout] glm52 超时(%ds),重试一次" % to)
                continue
            raise
    if last_err:
        raise last_err
...
def _official_ref_rank(cross):
    """官方 biggest-gainers / actives → 仅 ref_rank,不进主榜。"""
    official = set()
    for x in (cross.get("market_movers") or []):
        if isinstance(x, dict) and x.get("side") == "gainers" and x.get("symbol"):
            official.add(str(x["symbol"]).upper())
    for x in (cross.get("most_active") or []):
        if isinstance(x, dict) and x.get("symbol") and (x.get("chg_pct") or 0) > 0:
            official.add(str(x["symbol"]).upper())
    cross["ref_rank"] = sorted(official)
    return official


def _self_ret1d_board(cross):
    """自算 RET 1D = quote.price vs previousClose;过价格地板 + adv20。不读 platform.db / 8600。"""
    if isinstance(cross.get("self_ret1d_board"), list):
        return [x for x in cross["self_ret1d_board"] if isinstance(x, dict) and x.get("symbol")]
    pf, af = fetchers.pool_price_floor(), fetchers.pool_adv_floor_usd()
    # 处决案 $50M 宇宙地板与 build_universe 对齐;候选池 adv 仍走 pool_adv
    univ_af = float(os.getenv("UNIV_DOLLAR_VOL_MIN", "50000000"))
    floor_adv = min(af, univ_af)
    cands = []
    seen = set()
    for key in ("amc_tonight", "bmo_tomorrow", "upcoming_earnings", "earnings_movers",
                "candidate_pool", "market_movers", "most_active"):
        for x in (cross.get(key) or []):
            if not isinstance(x, dict):
                continue
    ...
    snaps = {}
    try:
        snaps = fetchers.quote_layer_snapshot(sorted(seen)[:80], with_rsi=True) if seen else {}
    except Exception as exc:
        print("[scout] self_ret1d snapshot:", str(exc)[:120])
    out = []
    for x in cands:
        sym = str(x.get("symbol") or "").upper()
        snap = snaps.get(sym) or {}
        px = snap.get("price") or x.get("price")
        adv = snap.get("adv20_usd") if snap.get("adv20_usd") is not None else x.get("adv20_usd")
        try:
            px = float(px) if px is not None else None
        except (TypeError, ValueError):
            continue
        if px is None or px < pf:
            continue
        if adv is None or float(adv) < floor_adv:
            continue
        if snap.get("chg_pct") is not None:
            ret = float(snap["chg_pct"]) / 100.0
        else:
            prev = snap.get("prev_close") or x.get("prev_close")
            try:
                prev = float(prev) if prev else None
            except (TypeError, ValueError):
                prev = None
            ret = ((px - prev) / prev) if prev else None
        if ret is None:
            continue
        out.append({"symbol": sym, "ret1d": ret, "price": px, "adv20_usd": adv})
    out.sort(key=lambda r: -(r.get("ret1d") or 0))
    cross["self_ret1d_board"] = out
    return out


def fmp_board_syms(cross):
    """主榜 = 自算 RET 1D(过地板) ∪ trend_board。官方涨幅/活跃只写 ref_rank。"""
    _official_ref_rank(cross)
    out = set()
    for x in _self_ret1d_board(cross):
        if isinstance(x, dict) and x.get("symbol"):
            out.add(str(x["symbol"]).upper())
    for x in (cross.get("trend_board") or []):
        if isinstance(x, dict) and x.get("symbol"):
            out.add(str(x["symbol"]).upper())
    return out
...
def glm_land_or_die(date, mode):
    """公开落盘硬闸。DS json/md/ds.html 只是半成品;无 GLM 编译章禁止当作已落盘。
    调用 scripts/scout_land_option.py(写 {mode}-final.md + 公开 html + OPTION emit)。失败非零退出。"""
    if mode not in GLM_LAND_SHIFTS:
        raise SystemExit("glm_land_or_die 不接受 mode=%s" % mode)
    here = os.path.dirname(os.path.abspath(__file__))
    script = os.path.normpath(os.path.join(here, "..", "scripts", "scout_land_option.py"))
    if not os.path.isfile(script):
        raise SystemExit("GLM land 脚本缺失:%s——禁止把 DS 直出当落盘" % script)
    print("[scout] GLM land 硬闸 →", script, date, mode)
    r = subprocess.run([sys.executable, script, "--date", date, "--mode", mode], cwd=here)
    if r.returncode != 0:
        raise SystemExit("GLM land 失败 exit=%s mode=%s——禁止把 DS 直出当落盘" % (r.returncode, mode))
    final = os.path.join(OUT, "briefs", "%s-%s-final.md" % (date, mode))
    htmlp = os.path.join(OUT, "briefs", "%s-%s.html" % (date, mode))
    if not os.path.isfile(final):
        raise SystemExit("缺 %s-final.md——禁止落盘" % mode)
    txt = open(final, encoding="utf-8").read()
    if "review_origin=glm52_cloud" not in txt:
        raise SystemExit("%s final 无 glm52_cloud 编译章——禁止落盘" % mode)
    if not os.path.isfile(htmlp):
        raise SystemExit("缺公开 html——禁止落盘")
    htxt = open(htmlp, encoding="utf-8").read()
    if "review_origin=glm52_cloud" not in htxt or "GLM 编译终稿" not in htxt:
        raise SystemExit("公开 html 未吃 GLM 终稿——禁止落盘")
    print("[scout] GLM 编译已落盘:", final, "+", htmlp)


def glm_morning_land_or_die(date):
    """晨报别名。verify / 旧调用点走这里,内部仍是 glm_land_or_die(date, morning)。"""
    return glm_land_or_die(date, "morning")
...
def evening_ds_with_lint(prompt, payload):
    ...
    prompt 纪律是软约束,这里是牙。"""
    return lint_evening_source_claims(denoise(glm_officer_call(prompt)), payload)
...
.pill.kill{border-color:var(--red);color:var(--red)}
.pill.watch{border-color:var(--gold);color:var(--gold)}
.badge.kill{border-color:var(--red);color:var(--red)}
.badge.watch{border-color:var(--gold);color:var(--gold)}
...
details.engine-fold{margin:12px 12px 14px;border:1px solid var(--line);border-radius:12px;
background:var(--panel);padding:10px 14px}
details.engine-fold>summary{color:var(--dim);font-size:var(--f1);letter-spacing:.08em;
list-style:none}
details.engine-fold>summary::-webkit-details-marker{display:none}
details.engine-fold[open]>summary{margin-bottom:8px;color:var(--ink)}
...
def _render_structured(date, data, mode="morning"):
    m = data.get("macro") or {}
    cands = data.get("candidates") or []
    ...
    banner = _SHIFT_TITLE.get(mode) or ("晚报复盘" if mode == "evening" else "任务单")
    out = ['<div class="banner"><b>%s</b><span class="num">%s</span>'
           '<span>SP500 <b>%s</b></span><span>NASDAQ <b>%s</b></span><span>置信 <b>%s</b></span>'
           '<span>出货风险 <b style="color:%s">%s</b></span></div>'
           % (_esc(banner), _esc(date), _esc(m.get("sp500_bias", "—")), _esc(m.get("nasdaq_bias", "—")),
...
_GLM_SLOT_ROW_RX = re.compile(
    r"^\|\s*(\d+)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*(.*?)\s*\|?\s*$"
)


def parse_glm_slots(md):
    """从 GLM 终稿 Markdown 表抽出四槽。作废/观察也占槽,不是把卡删掉。"""
    by = {}
    for ln in (md or "").splitlines():
        t = ln.strip()
        if not t.startswith("|"):
            continue
        if re.match(r"^\|\s*-+", t) or re.match(r"^\|\s*Slot\s*\|", t, re.I):
            continue
        m = _GLM_SLOT_ROW_RX.match(t)
        if not m:
            continue
        slot = int(m.group(1))
        if slot < 1 or slot > 4:
            continue
        raw_t = m.group(3).strip()
        ticker = re.sub(r"[^A-Za-z]", "", raw_t).upper()
        status = re.sub(r"[*_]", "", m.group(5)).strip()
        by[slot] = {
            "slot": slot,
            "slot_name": m.group(2).strip(),
            "ticker": ticker,
            "direction": re.sub(r"[*_]", "", m.group(4)).strip().lower() or "call",
            "status": status,
            "note": m.group(6).strip(),
        }
    return [by[i] for i in range(1, 5) if i in by]


def _glm_status_kind(status):
    s = re.sub(r"\s+", "", status or "")
    if "可执行" in s:
        return "live"
    if "作废" in s or "空槽" in s:
        return "kill"
    if "观察" in s or "不建仓" in s:
        return "watch"
    return "watch"


def _glm_section(md, *titles):
    text = md or ""
    parts = re.split(r"(?m)^##\s+", text)
    want = {t.replace(" ", "") for t in titles}
    for p in parts[1:]:
...
def _ds_cand_for_glm(data, ticker):
    t = (ticker or "").upper()
    if not t:
        return None
    for c in (data or {}).get("candidates") or []:
        if isinstance(c, dict) and (c.get("ticker") or "").upper() == t:
            return c
    return None


def _glm_slot_card(row, ds_cand=None):
    """一张槽位卡。面值=GLM 状态;DS 只补形态/入场等细节,不得把作废卡画成可做 CALL。"""
    kind = _glm_status_kind(row.get("status"))
    sl = row.get("slot") or "?"
    sn = row.get("slot_name") or ""
    ticker = (row.get("ticker") or "").upper() or "空"
    status = row.get("status") or ""
    note = row.get("note") or ""
    d = (row.get("direction") or "call").lower()
    if kind == "kill":
        pill_cls, pill, badge_cls, badge = " kill", "作废", " kill", "S%s %s · GLM作废" % (sl, sn)
    elif kind == "watch":
        pill_cls, pill, badge_cls, badge = " watch", "观察", " watch", "S%s %s · GLM观察" % (sl, sn)
    else:
        pill_cls = " put" if d == "put" else ""
        pill, badge_cls, badge = d.upper(), "", "S%s %s · GLM可执行" % (sl, sn)
    rows = [("GLM 状态", status), ("GLM 读数", note)]
    ds_rows, evs = [], ""
    if ds_cand and not ds_cand.get("empty"):
        st = ds_cand.get("strategy") or {}
        ds_rows = [(lab, st.get(key)) for lab, key in (
            ("形态", "type"), ("入场条件", "entry_condition"),
            ("入场窗(PST)", "entry_window_pst"), ("出场窗(PST)", "exit_window_pst"),
            ("止损", "stop"), ("作废条件", "abandon"),
            ("行权价逻辑", "strike_logic"), ("到期", "expiry"),
        ) if st.get(key)]
        if kind == "live":
            rows.extend(ds_rows)
            ev_html = "".join(
                '<div class="ev"><div class="src">%s</div><div>%s</div><div class="why">%s</div></div>'
                % (_esc(e.get("source", "")), _esc(e.get("item", "")), _esc(e.get("why", "")))
                for e in (ds_cand.get("evidence") or [])
            )
            if ev_html:
                evs = "<details><summary>展开解析(证据出处)</summary>%s</details>" % ev_html
        elif ds_rows:
            ds_kvs = "".join('<div class="kv"><b>%s</b><span>%s</span></div>' % (_esc(l), _esc(v))
                             for l, v in ds_rows)
            evs = "<details><summary>DS 原卡细节(面值以 GLM 为准)</summary>%s</details>" % ds_kvs
    kvs = "".join('<div class="kv"><b>%s</b><span>%s</span></div>' % (_esc(k), _esc(v))
                  for k, v in rows if v)
    return ('<div class="card"><span class="badge%s">%s</span>'
            '<div class="tick"><span class="sym">%s</span><span class="pill%s">%s</span></div>'
            '%s%s</div>' % (badge_cls, _esc(badge), _esc(ticker), pill_cls, _esc(pill), kvs, evs))


def _render_glm_job(date, mode, glm_md, data=None):
    """公开作业:GLM 结论 + 四槽卡。无四槽表才降级纯 Markdown(晚报常见)。"""
    slots = parse_glm_slots(glm_md)
    conclusion = _glm_section(glm_md, "编译结论")
    lyra = _glm_section(glm_md, "一句话给 Lyra", "一句话")
    risks = _glm_section(glm_md, "风险")
    if not slots:
        return _md_fallback(glm_md)
    banner = _SHIFT_TITLE.get(mode) or mode
    out = ['<div class="banner"><b>%s</b><span class="num">%s</span>'
           '<span>面值=GLM 四槽卡</span></div>' % (_esc(banner), _esc(date))]
    if conclusion:
        out.append('<section><h2>一 · 编译结论</h2><div class="card">%s</div></section>'
                   % "".join("<p>%s</p>" % _esc(p) for p in conclusion.split("\n\n") if p.strip()))
    cells = []
    for i in range(1, 5):
        row = next((s for s in slots if s["slot"] == i), None)
        if not row:
            cells.append('<div class="empty">S%s · 今日空槽<br><span style="font-size:var(--f1)">GLM 表未写本槽</span></div>' % i)
            continue
        cells.append(_glm_slot_card(row, _ds_cand_for_glm(data, row.get("ticker"))))
    out.append('<section><h2>二 · 四槽股票卡(GLM 终稿)</h2>%s</section>' % "".join(cells))
    if risks:
        items = [ln.lstrip("- ").strip() for ln in risks.splitlines() if ln.strip()]
        out.append('<section><h2>三 · 风险</h2><div class="card">%s</div></section>'
                   % "".join("<p>%s</p>" % _esc(x) for x in items))
    if lyra:
        out.append('<section><h2>四 · 一句话给 Lyra</h2><div class="card">%s</div></section>'
                   % _esc(lyra.split("\n\n")[0].strip()))
    return "".join(out)
...
def _engine_card(cross):
    ...
    vix = cross.get("vix_chg_pct")
    down = "%s/%s" % (cross.get("down_count"), cross.get("of"))
    liq = "WATCH" if cross.get("liquidation_watch") else "未触发全线下跌"
    summary = "〇 · 跨资产引擎读数 · VIX %s%% · 收跌 %s · %s · %d 行（点击展开）" % (
        vix if vix is not None else "—", down, liq, len(rows))
    return ('<details class="engine-fold"><summary>%s</summary>'
            '<div class="card">%s</div></details>' % (_esc(summary), kvs))


def render_glm_brief_html(date, mode, glm_md, cross=None, data=None):
    """公开 HTML = GLM 终稿四槽卡 + 折叠引擎墙。禁止把未编译的 DS 卡当公开作业。"""
    warn = ""
    ...
    banner = ('<div class="banner"><b>GLM 编译终稿</b><span class="num">%s</span>'
              '<span>review_origin=glm52_cloud</span><span>%s</span></div>'
              % (_esc(date), _esc(_SHIFT_HTML_LABEL.get(mode, mode))))
    job = _render_glm_job(date, mode, glm_md or "", data)
    body = warn + banner + job + _engine_card(cross)
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            '<meta name="viewport" content="width=device-width,initial-scale=1"/>'
            '<title>SCOUT · %s · %s</title><style>%s</style></head><body>'
            '<header><h1>SCOUT</h1><span class="sub">%s · %s · GLM 终稿 · 参谋作业,Lyra 拍板</span></header>'
            '%s<footer>build scout v3.31 · 公开页=GLM终稿 · DS半成品=.ds.html · '
            '反馈回路+事件层+六因子核+准入闸</footer></body></html>'
            % (mode.upper(), _esc(date), BRIEF_CSS, mode, _esc(date), body))


def write_public_html_from_glm(date, mode, glm_md, cross=None, data=None):
    bdir = os.path.join(OUT, "briefs")
    os.makedirs(bdir, exist_ok=True)
    hp = os.path.join(bdir, "%s-%s.html" % (date, mode))
    with open(hp, "w", encoding="utf-8") as f:
        f.write(render_glm_brief_html(date, mode, glm_md, cross, data))
    print("[scout] 公开 html 已改吃 GLM 终稿:", hp)
    return hp


def _pending_glm_stub_html(date, mode):
    msg = ("本班 DS 半成品已写入 %s-%s.ds.html / .json / .md。"
           "公开作业必须等 GLM 编译。无编译章禁止当落盘。"
           % (date, mode))
    warn = ('<div class="banner" style="border-color:var(--red)">'
            '<b style="color:var(--red)">待 GLM 编译</b><span>%s</span></div>' % _esc(msg))
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            '<title>SCOUT · PENDING GLM · %s</title><style>%s</style></head><body>'
            '<header><h1>SCOUT</h1><span class="sub">%s · %s · 尚未落盘</span></header>'
            '%s</body></html>' % (_esc(date), BRIEF_CSS, mode, _esc(date), warn))
...
def render_brief_html(date, mode, data, raw_text, cross=None):
    ...
    job = _render_structured(date, data, mode) if data else _md_fallback(raw_text)
    body = warn + job + _engine_card(cross)
    return ('<!DOCTYPE html><html lang="zh"><head><meta charset="utf-8"/>'
            '<meta name="viewport" content="width=device-width,initial-scale=1"/>'
            '<title>SCOUT · %s · %s</title><style>%s</style></head><body>'
...
def render_console(title, body, date, mode, data=None, cross=None):
    ...
    try:
        ...
        t = _http(CONSOLE + "/api/tasks",
                  {"workspace": "trade", "title": title, "owner_node": "glm52_lane",
    ...
    with open(bp, "w", encoding="utf-8") as f:
        f.write("# %s\n\n%s\n" % (title, body))
    ds_hp = os.path.join(bdir, "%s-%s.ds.html" % (date, mode))
    hp = os.path.join(bdir, "%s-%s.html" % (date, mode))
    ds_html = render_brief_html(date, mode, data, body, cross)
    if mode in GLM_LAND_SHIFTS or mode == "evening":
        # DS 只许半成品;公开 html 先 stub,GLM land 后再覆盖为终稿
        with open(ds_hp, "w", encoding="utf-8") as f:
            f.write(ds_html)
        with open(hp, "w", encoding="utf-8") as f:
            f.write(_pending_glm_stub_html(date, mode))
    else:
        with open(hp, "w", encoding="utf-8") as f:
            f.write(ds_html)
    if data is not None:
    ...
    print("[scout] 落盘:", bp, "+", hp, "(+json)" if data is not None else "",
          "(DS→.ds.html)" if (mode in GLM_LAND_SHIFTS or mode == "evening") else "")
...
def main():
    ...
    if a.mode in ("morning", "midday", "earnings"):
        ...
        ds_err = ""
        try:
            body = glm_officer_call(prompt)
        except SystemExit:
            raise
        except Exception as e:   # v3.26.3:超时/网络类失败不崩班——失败标记+html 必须覆盖昨日页
            body, ds_err = "", "glm52 调用失败(%s)" % str(e)[:120]
            print("[scout]", ds_err)
        ...
        if data is None:
            hint = (ds_err or "DS 空正文(finish=length/未关 think 类,8-17 案)") if not (body or "").strip() \
                else "疑输出截断,查 GLM_OFFICER_MAX_TOKENS" if body.lstrip().startswith("{") \
                else "DS 未按 schema"
        ...
        if shift in GLM_LAND_SHIFTS:
            glm_land_or_die(today, shift)   # GLM 编译后才算落盘(公开 html+emit 都在 land 内)
        else:
        ...
        pf = os.path.join(OUT, "briefs", "%s-evening-final.md" % today)
        write_public_html_from_glm(today, "evening", open(pf, encoding="utf-8").read(), cross)
