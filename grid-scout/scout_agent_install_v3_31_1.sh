#!/usr/bin/env bash
# ============================================================================
# Scout Agent · v3.31.1 · 总包(完整替代;plist/verify 保护;自检自跑;落地后自归档一份进仓)
#   ★ v3.31.1(2026-08-26,Lyra 拍:五个 plist 全 3.13):plist 模板解释器改占位 __PYTHON__,落地时按 PYTHON_BIN 或
#     command -v python3.13 解析(解析不到=响亮报,不猜路径);既有 plist 仍不覆盖,给戌的对齐命令见 README
#   ★ v3.31.0(2026-08-26 午):反馈回路——归因账本 + 因子 IC 表 + 期权中价结算(Theta 历史)+ 数据自检(Alpaca 第二源)
#     + 同团封顶 + 事件时刻窗;新文件 attribution.py;verify 60;冒烟案 T
#   ★ v3.30.0(2026-08-26 午,Lyra:NVDA 财报/关键数据/美债/油一个没考虑):事件层——FMP 经济日历(官方端点)+ 巨头财报 + 10Y + WTI
#     → 事件日:仓位档降一档、T+0 节奏线 1.0、卡带事件前缀、持过巨头夜的财报腿标 gap 风险;verify 59;冒烟案 S(8-26 实况)
#   ★ v3.29.3(2026-08-26 中午班实况):卡上 ET 数字贴 PST 标签(13:45–14:30 入场)→ 引擎归一化/窗外作废;
#     无放量 T+0 call(TEM vol_x20 0.44)→ 按班次时刻折算节奏 <0.8 作废只观察;晨午班 t0 视角权重;verify 58;冒烟案 R
...
# v3.31.1:plist 模板解释器(Lyra 拍全 3.13)。不猜路径:PYTHON_BIN 优先,其次 command -v python3.13;都无 → 模板留占位并响亮报
PYTHON_BIN="${PYTHON_BIN:-$(command -v python3.13 || true)}"
if [ -z "$PYTHON_BIN" ]; then
  echo "[install] ⚠ 未找到 python3.13(PYTHON_BIN 未设且 PATH 无 python3.13):plist.new 模板保留 __PYTHON__ 占位,加载前手工替换"
...
<plist version="1.0"><dict>
  ...
  <array><string>__PYTHON__</string><string>__SCOUT_DIR__/scout_agent.py</string>
...
PKG_EOF_001
sed -i.bak "s#__PYTHON__#$PYTHON_BIN#g" .tmp_001 && rm -f .tmp_001.bak
...
PKG_EOF_002
sed -i.bak "s#__PYTHON__#$PYTHON_BIN#g" .tmp_002 && rm -f .tmp_002.bak
...
PKG_EOF_007
sed -i.bak "s#__PYTHON__#$PYTHON_BIN#g" .tmp_007 && rm -f .tmp_007.bak
...
PKG_EOF_008
sed -i.bak "s#__PYTHON__#$PYTHON_BIN#g" .tmp_008 && rm -f .tmp_008.bak
...
PKG_EOF_009
sed -i.bak "s#__PYTHON__#$PYTHON_BIN#g" .tmp_009 && rm -f .tmp_009.bak
...
if __name__ == "__main__":
    main()
PKG_EOF_010
cat > 'attribution.py' <<'PKG_EOF_012'
...
def theta_option_mid_at(symbol, expiration, spot, t_in_pst, t_out_pst, interval="5m"):
    """v3.31 期权结算:Theta 历史 greeks/all(官方文档形态:/v3/option/history/greeks/all?symbol=X&expiration=YYYYMMDD&date=YYYYMMDD&interval=5m,
    行含 bid/ask/strike/right 与时间戳)——取 ATM call 在 t_in / t_out(PST,转 ET)的中价。
    时间戳字段名按回包自证(ms_of_day / timestamp / time 任一),首行键名异常写 skip;失败 → None,不装数。"""
    if not _theta_up():
        _log_skip("theta_option_mid", symbol, "Theta Terminal 未起"); return None
    exp = str(expiration).replace("-", "")
    q = {"symbol": symbol, "expiration": exp, "date": exp, "interval": interval, "right": "call", "strike_range": "1", "format": "json"}
    url = _theta_base() + "/v3/option/history/greeks/all?" + urllib.parse.urlencode(q)
    try:
        data = json.loads(_get(url))
    except Exception as e:
        _log_skip("theta_option_mid", symbol, "历史 greeks 请求失败:%s" % str(e)[:100]); return None
    rows = data if isinstance(data, list) else (data.get("data") if isinstance(data, dict) else None)
    if not rows:
        _log_skip("theta_option_mid", symbol, "历史 greeks 空回包"); return None
    r0 = rows[0]
    tkey = next((k for k in ("ms_of_day", "timestamp", "time", "ms") if k in r0), None)
    if tkey is None or "strike" not in r0 or ("bid" not in r0 and "ask" not in r0):
        _log_skip("theta_option_mid", symbol, "字段形状异常,首行键:%s" % sorted(r0.keys())[:14]); return None
    def _ms(v):
        try:
            if tkey == "ms_of_day": return int(v)
            s_ = str(v)
            if "T" in s_ or " " in s_:
                hh, mm = s_.replace("T", " ").split(" ")[1].split(":")[:2]
                return (int(hh) * 60 + int(mm)) * 60000
            return int(v)
        except Exception:
            return None
    # ATM = 与 spot 最近的 strike(同一 expiration)
    strikes = sorted({float(r.get("strike")) for r in rows if r.get("strike") is not None})
    if not strikes:
        _log_skip("theta_option_mid", symbol, "无 strike"); return None
    k = min(strikes, key=lambda x: abs(x - float(spot)))
    def _mid_at(hm_pst):
        target = ((hm_pst[0] + 3) * 60 + hm_pst[1]) * 60000
        best = None
        for r in rows:
            if float(r.get("strike") or -1) != k or str(r.get("right") or "call").lower() not in ("call", "c"):
                continue
            ms = _ms(r.get(tkey))
            if ms is None or ms > target + 10 * 60000:
                continue
            if best is None or ms > best[0]:
                best = (ms, r)
        if not best:
            return None
        b, a = float(best[1].get("bid") or 0), float(best[1].get("ask") or 0)
        return (b + a) / 2 if (b and a) else None
    return {"strike": k, "mid_in": _mid_at(t_in_pst), "mid_out": _mid_at(t_out_pst)}
...
def fetch_economic_calendar():
    """v3.30 事件层:FMP stable /economic-calendar?from&to(官方文档 2026-08-26 实读:site.financialmodelingprep.com/developer/docs/stable/economics-calendar)。
    取 今日−1 … 今日+3;只留 US;字段名按回包自证(event/country/date/impact/actual/estimate/previous),
    首行键名写进 skip 供对账;失败=源级失败(ok:false),不装数。"""
    def go():
        key = _fmp_key()
        if not key:
            _log_skip("economic_calendar", "-", "无 FMP key"); return []
        t = trading_date()
        q = {"from": (t - datetime.timedelta(days=1)).isoformat(), "to": (t + datetime.timedelta(days=3)).isoformat(), "apikey": key}
        _fmp_tick()
        rows = json.loads(_get(_FMP_STABLE_BASE + "/economic-calendar?" + urllib.parse.urlencode(q)))
        if not isinstance(rows, list):
            _log_skip("economic_calendar", "-", "回包非列表:%s" % str(rows)[:120]); return []
        if rows and not {"event", "date"} <= set(rows[0].keys()):
            _log_skip("economic_calendar", "-", "字段形状异常,首行键:%s" % sorted(rows[0].keys())[:12])
        out = []
        for r in rows:
            try:
                if str(r.get("country") or "").upper() not in ("US", "USA", "UNITED STATES"):
                    continue
                out.append({"date": str(r.get("date") or "")[:16], "event": str(r.get("event") or "")[:80],
                            "impact": str(r.get("impact") or "")[:10], "actual": r.get("actual"),
                            "estimate": r.get("estimate"), "previous": r.get("previous"), "unit": r.get("unit")})
            except Exception:
                continue
        return out
    return _wrap("economic_calendar", go)
...
# v3.31.1:自检与门禁用 plist 同一解释器跑(Lyra 拍全 3.13);PYTHON_BIN 解析不到时退回 python3 并由门禁自己报红
SELF_PY="$PYTHON_BIN"; [ "$SELF_PY" = "__PYTHON__" ] && SELF_PY=python3
"$SELF_PY" - <<'SELFCHECK'
...
if r.returncode == 0:
    ...
    open(".gate_red", "w").write((r.stdout + r.stderr)[-2000:])
...
cp -f "$SELF_PATH" "./scout_agent_install_v3_31_1.sh" 2>/dev/null && echo "[install] 安装脚本已归档 ./scout_agent_install_v3_31_1.sh" || true
if [ -f .gate_red ]; then
  echo ""; echo "⚠ Scout Agent v3.31.1 文件已落地,但门禁红(见上)——按资格线这不算部署完成;修红后重跑 $SELF_PY verify_scout_deploy.py"
  rm -f .gate_red
else
  echo ""; echo "✓ Scout Agent v3.31.1: $(pwd) — 门禁绿;首跑 $SELF_PY scout_agent.py --mode morning"
fi
