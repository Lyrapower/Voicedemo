...
def _stooq_daily(sym, name, skip_src):
    ...
    try:
        ...
        pc = float(prev["Close"])
        ci, oi = head.index("Close"), head.index("Open")
        vi = head.index("Volume") if "Volume" in head else None
        tail = rows[1:] if len(rows) <= 261 else rows[-260:]
        opens, closes, vols = [], [], []
        for r in tail:
            pcs = r.split(",")
            try:
                o_, c_ = float(pcs[oi]), float(pcs[ci])
            except (ValueError, IndexError):
                continue
            opens.append(o_); closes.append(c_)
            v_ = None
            if vi is not None:
                try:
                    v_ = float(pcs[vi]) or None
                except (ValueError, IndexError):
                    v_ = None
            vols.append(v_)
    ...
    def go():
        out = [_stooq_daily(sym, name, "indices")
               for sym, name in (("^spx", "SP500"), ("^ndq", "NASDAQ"), ("^vix", "VIX"),
                                 ("rsp.us", "RSP"),    # RSP 等权:对 SP500 的 20日差=广度代理
                                 ("hyg.us", "HYG"), ("lqd.us", "LQD"))]  # 信用金丝雀:垃圾债 vs 投级
