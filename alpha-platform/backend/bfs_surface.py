"""BFS store vs surface filter — shared reconcile (Alpha 8600 ↔ Aether 8501).

Store/worker: all bid/ask rows入库, no watchlist filter.
Surface: watchlist + SURFACE_DENY only on display (8501 决策面).
"""
...
def reconcile_bfs_rows(
    ...
    *,
    ...
    """raw_n = bid/ask 可入库; surface_n = 决策面可见; filtered = 滤除(可对账 raw=surface+len(surface_filtered))."""
